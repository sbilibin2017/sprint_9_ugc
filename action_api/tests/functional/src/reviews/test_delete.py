...
async def test_review_delete(
    ...
    response = await make_request(
        method="delete", api_method=f"reviews/{movie_id}", jwt_token=jwt_token
    )
...
@pytest.mark.parametrize(
    "movie_id,jwt_token",
    testdata.REVIEW_DELETE_NO_DATA,
)
async def test_review_delete_no_data(
    movie_id: uuid.UUID,
    jwt_token: str,
    make_request: tests_models.MakeResponseCallableType,
):
...
@pytest.mark.parametrize(
    "create_body,movie_id,jwt_token",
    testdata.REVIEW_DELETE_INVALID,
)
async def test_review_delete_invalid(
    create_body: dict[str, typing.Any],
    movie_id: uuid.UUID,
    jwt_token: str,
    make_request: tests_models.MakeResponseCallableType,
